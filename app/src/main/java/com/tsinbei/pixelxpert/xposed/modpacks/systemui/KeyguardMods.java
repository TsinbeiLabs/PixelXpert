...
import static com.tsinbei.pixelxpert.xposed.utils.reflection.XposedCompat.getBooleanField;
...
import static com.tsinbei.pixelxpert.xposed.modpacks.systemui.BatteryDataProvider.isCharging;
import static com.tsinbei.pixelxpert.xposed.utils.SystemUtils.idOf;
import static com.tsinbei.pixelxpert.xposed.utils.reflection.HookHelper.callMethod;
import static com.tsinbei.pixelxpert.xposed.utils.reflection.HookHelper.getObjectField;
import static com.tsinbei.pixelxpert.xposed.utils.toolkit.ColorUtils.getColorAttrDefaultColor;
...
import com.tsinbei.pixelxpert.xposed.utils.StringFormatter;
import com.tsinbei.pixelxpert.xposed.utils.SystemUtils;
import com.tsinbei.pixelxpert.xposed.utils.reflection.HookHelper;
import com.tsinbei.pixelxpert.xposed.utils.reflection.ReflectedClass;
import com.tsinbei.pixelxpert.xposed.utils.reflection.ReflectionTools;
...
//		clockStringFormatter.registerCallback(this::updateMiddleTexts);
		...
		CarrierTextControllerClass
				...
				.run(param -> {
					...
					ReflectedClass.of(carrierTextCallback.getClass())
							...
							.run(param1 -> {
								if (customCarrierTextEnabled && param1.args.length > 0 && param1.args[0] != null) {
									setObjectField(
											param1.args[0],
											"carrierText",
											trimInvisibleEdges(carrierStringFormatter.formatString(customCarrierText)));
								}
							});
					setCarrierText();
				});
	...
	private void setCarrierText() {
		try {
			applyCarrierTextOffset();
			callMethod(getObjectField(carrierTextController, "mCarrierTextManager"), "updateCarrierText");
		} catch (Throwable ignored) {} //probably not initiated yet
	}

	private void applyCarrierTextOffset() {
		try {
			TextView view = getObjectField(carrierTextController, "mView");
			float offset = customCarrierTextEnabled
					? -TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 8, mContext.getResources().getDisplayMetrics())
					: 0;
			view.setTranslationX(view.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL ? -offset : offset);
		} catch (Throwable ignored) {}
	}

	private static CharSequence trimInvisibleEdges(CharSequence text) {
		int start = 0;
		int end = text.length();

		while (start < end && isInvisibleEdgeCharacter(text.charAt(start))) start++;
		while (end > start && isInvisibleEdgeCharacter(text.charAt(end - 1))) end--;

		return text.subSequence(start, end);
	}

	private static boolean isInvisibleEdgeCharacter(char character) {
